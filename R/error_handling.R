...
prep_id <- function(hospital_id = NULL) {

  tryCatch(stopifnot(rlang::is_empty(hospital_id) == FALSE,
...
check_output_format <- function(output_format = NULL) {
  ...
  tryCatch(stopifnot(rlang::is_empty(output_format) == FALSE,
...
check_data_type <- function(data_type = NULL) {

  tryCatch(stopifnot(rlang::is_empty(data_type) == FALSE,
...
prep_sleep_time <- function(sleep_time = NULL) {
  ...
  tryCatch(stopifnot(rlang::is_empty(sleep_time) == FALSE,
                     is.na(sleep_time) == FALSE,
                     grepl(pattern = "^(\\s+)?[0-9]+(\\s+)?$", x = as.character(sleep_time))),
           error = function(c) {

             error_msg_varying <- dplyr::case_when(
               grepl(pattern = "empty", x = c) ~ "sleep_time can't be a NULL object, it must be a numerical object.",
